...
`[log_d_hor, v_hor, target_bearing_world, velocity_direction_body, delta_z, body_vz, yaw]`

- `log_d_hor` — `log(水平目标距离 + 1)`
- `v_hor` — 机体系水平速度大小
- `target_bearing_world` — 世界系目标方位角
- `velocity_direction_body` — 机体系水平速度方向角
- `delta_z` — AirSim NED 下的目标高度差 `goal_z - current_z`
- `body_vz` — AirSim NED 下的机体系垂直速度（向下为正）
- `yaw` — 世界系偏航角

状态完整保留 AirSim NED 坐标语义，不做坐标符号转换，也不做固定区间归一化。
...
| 维度 | 含义 | 范围 |
|------|------|------|
| `body_vx` | 机体系 x 轴速度 | [-2.0, 2.0] m/s |
| `body_vy` | 机体系 y 轴速度 | [-2.0, 2.0] m/s |
| `body_vz` | 机体系 z 轴速度（NED 向下为正） | [-2.0, 2.0] m/s |

### 奖励函数

当前稠密奖励由以下原始惩罚值直接相加，暂未应用单独权重：

| 奖励项 | 当前计算方式 |
|--------|-------------|
| **水平距离** | `-log(水平目标距离 + 1)` |
| **垂直距离** | `-|目标高度差|` |
| **目标方向** | `-|机体系速度方向 + yaw - 目标方位角|`（角度环绕到 `[-π, π)`） |
| **前向飞行** | `-|机体系水平速度方向|` |
| **平滑性** | `-||当前世界系速度 - 上一步世界系速度||₂` |
| **速度上限** | 不启用；动作空间已限制速度范围 |
| **成功 / 碰撞 / 超时** | `+20 / -20 / -30` |

原有速度投影、曲率、步惩罚、停滞、近障、进度和软高度惩罚代码仍会计算并记录，但不计入总奖励；硬高度边界仍作为安全终止条件。
