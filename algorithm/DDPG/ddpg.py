...
class DDPGAgent:
    def __init__(self, base_dim: int, depth_shape, action_space, args, device=None, seed=None):
        ...
        self.tau = args.tau
        # DDPG: No policy noise, noise clip, or policy freq needed
        # (These are TD3 specific)
    ...
    def train(self, progress_ratio=0.0):
        ...
        with torch.no_grad():
            ...
            next_actions = self.actor_target(next_states_actor).clamp(-1.0, 1.0)

            # DDPG: Use single Q-value (no double Q-learning)
            target_Q, _ = self.critic_target(next_states_critic, next_actions)
            target_Q = rewards + (1 - dones) * self.gamma * target_Q

        # DDPG: Use single Q-value for critic loss
        current_Q, _ = self.critic(states_critic, actions)
        ...
        # Use fixed critic features while still letting dQ/da flow through the
        # critic to the actor action.
        with torch.no_grad():
...
def make_agent(env, initial_obs, args, device=None) -> DDPGAgent:
