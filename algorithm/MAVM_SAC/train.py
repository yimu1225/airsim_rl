#!/usr/bin/env python3
"""Independent staged trainer for MAVM-SAC.

Run this file directly to avoid importing the project's monolithic training
entry point::

    python algorithm/MAVM_SAC/train.py --help
"""
...
if __package__ in (None, ""):
    ...
    __package__ = "algorithm.MAVM_SAC"
...
from .agent import MAVMSACAgent
...
def _validate_observation(config: MAVMConfig, observation: dict[str, np.ndarray]) -> None:
    ...
    if observation["depth"].shape != expected:
        raise ValueError(
            ...
            "update the MAVM params.yaml image dimensions"
        )
...
def _save_agent(agent: MAVMSACAgent, output: Path, stage: str, step: int) -> None:
...
def _environment_train(
    ...
    *,
    ...
    if stage == "sac":
        algorithm_name = "CL-MAVM-SAC" if args.curriculum else "MAVM-SAC"
...
def _sum_memory_offset_losses(losses: list[torch.Tensor]) -> torch.Tensor:
    """Match MAVRL by summing enabled temporal-target losses."""
    if not losses:
...
def _memory_stage_config(
    vision_checkpoint_config: dict[str, Any], requested_config: MAVMConfig
) -> MAVMConfig:
    config = MAVMConfig.from_mapping(vision_checkpoint_config)
...
def train_vision(args: argparse.Namespace, _unknown_args: list[str]) -> None:
    ...
    if tuple(dataset[0]["depth"].shape) != (
        ...
        raise ValueError("dataset frame shape does not match the MAVM configuration")
...
def train_memory(args: argparse.Namespace, _unknown_args: list[str]) -> None:
    ...
    requested_config = MAVMConfig.load(args.config)
    ...
    step = 0
    validation = DataLoader(
        EpisodeSequenceDataset(args.dataset, sequence_length=None, split="validation", seed=args.seed),
        batch_size=batch_size, shuffle=False, collate_fn=pad_episode_batch,
        **_offline_loader_options(args.workers, device),
    )
    try:
        for epoch in range(args.epochs):
            ...
            for batch in progress:
                ...
                batch_size, length, channels, height, width = frames.shape
                with torch.no_grad():
                    latent = encoder(frames.reshape(-1, channels, height, width))
                ...
                for offset in config.reconstruction_offsets:
                    if masks[offset].any():
                        ...
                        losses.append(_reconstruction_loss(
                            predictions[offset][masks[offset]],
                            targets[offset][masks[offset]],
                            config.reconstruction_loss,
                            charbonnier_epsilon=config.charbonnier_epsilon,
                        ))
                if not losses:
                ...
                torch.nn.utils.clip_grad_norm_(
                    [*memory.parameters(), *reconstructor.parameters()],
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Independent staged MAVM-SAC trainer",
    ...
    def common(subparser: argparse.ArgumentParser) -> None:
        ...
        subparser.add_argument("--output", default="runs/MAVM_SAC")
    ...
    memory = subparsers.add_parser("memory", help="stage 4: causal memory and t-5/t-10/t+5 decoder")
    common(memory)
...
def main(argv: list[str] | None = None) -> None:
    ...
    print(f"MAVM stage '{args.stage}' completed in {(time.time() - started) / 60:.1f} minutes")
