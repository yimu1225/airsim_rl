...
class STVimEncoder(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.vim_embed_dim = int(get_algo_param(args, "vmembed_dim", legacy_embed_dim))
        ...
        if self.base_dim <= 0:
            raise ValueError("MM_VSSM_SAC requires args.base_dim > 0")
        ...
        if int(configured_temporal_dim) != self.temporal_dim:
            raise ValueError(
                "MM_VSSM_SAC concatenates visual and base tokens directly, so "
                f"st_temporal_mamba_dim must equal vmembed_dim + st_base_proj_dim "
