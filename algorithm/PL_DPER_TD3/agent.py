...
class PLDPERTD3Agent:
    ...
    def _sync_cuda(self, stage: str):
        ...
        try:
            ...
            raise RuntimeError(f"CUDA failure during DPER-TD3 stage '{stage}': {exc}") from exc

    @staticmethod
    def _ensure_finite(tensor: torch.Tensor, name: str):
        if not torch.isfinite(tensor).all():
            raise RuntimeError(f"Non-finite tensor detected in DPER-TD3: {name}")
