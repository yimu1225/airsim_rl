...
class PLSB_PERTD3Agent:
    def __init__(self, base_dim: int, depth_shape, action_space, args, device=None, seed=None):
        ...
        self.critic_state_dim = self.base_dim + self.critic_encoder.repr_dim
        ...
        self.replay_buffer = DualPrioritizedReplayBuffer(
            ...
            success_capacity_ratio=get_algo_param(args, "sb_per_td3_success_capacity_ratio", 0.3),
            success_sample_ratio=float(get_algo_param(args, "sb_per_td3_mu_low", 0.30)),
            alpha=get_algo_param(args, "sb_per_td3_alpha", 0.6),
            eps=get_algo_param(args, "sb_per_td3_priority_eps", 1e-6),
            seed=seed,
        )
        ...
        self.sb_per_beta_start = get_algo_param(args, "sb_per_td3_beta_start", 0.4)
        self.sb_per_beta_final = get_algo_param(args, "sb_per_td3_beta_final", 1.0)

        # Staircase schedule for success-prioritized sampling ratio mu.
        self.mu_low = float(get_algo_param(args, "sb_per_td3_mu_low", 0.30))
        self.mu_mid = float(get_algo_param(args, "sb_per_td3_mu_mid", 0.40))
        self.mu_high = float(get_algo_param(args, "sb_per_td3_mu_high", 0.45))
        self.mu_step1 = float(get_algo_param(args, "sb_per_td3_mu_step1", 0.25))
        self.mu_step2 = float(get_algo_param(args, "sb_per_td3_mu_step2", 0.70))
    ...
    def _sync_cuda(self, stage: str):
        ...
        try:
            ...
            raise RuntimeError(f"CUDA failure during SB_PER-TD3 stage '{stage}': {exc}") from exc

    @staticmethod
    def _ensure_finite(tensor: torch.Tensor, name: str):
        if not torch.isfinite(tensor).all():
            raise RuntimeError(f"Non-finite tensor detected in SB_PER-TD3: {name}")
    ...
    def _concat_actor_state(self, base: torch.Tensor, depth: torch.Tensor, encoder_net, detach_encoder: bool = False) -> torch.Tensor:
    ...
    def _get_current_beta(self, progress_ratio: float) -> float:
        ...
        return self.sb_per_beta_start * (1.0 - p) + self.sb_per_beta_final * p
    ...
    def select_action(self, base_state, depth, noise: bool = True, progress_ratio: float = 0.0):
        ...
        with torch.no_grad():
            state = self._concat_actor_state(base_tensor, depth_tensor, self.actor_encoder)
...
SB_PERTD3Agent = PLSB_PERTD3Agent
