...
class SB_PERVSSM_TD3Agent:
    """
    VSSM_TD3 + PER:
    - Visual encoder: STVimEncoder
    - Critic update: importance-weighted TD loss
    - Replay: dual prioritized replay buffer (success/regular)
    """

    def __init__(self, base_dim, depth_shape, action_space, args, device=None, seed=None):
        self.device = torch.device(device if device is not None else ("cuda" if torch.cuda.is_available() else "cpu"))
        print(f"SB_PER-Vim-TD3 Agent using device: {self.device}")
