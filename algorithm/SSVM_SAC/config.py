"""Serializable model and optimization configuration for SSVM-SAC."""
...
class SSVMConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if any(value <= 0 for value in positive.values()):
            raise ValueError(f"SSVM dimensions and capacities must be positive: {positive}")
    ...
    @classmethod
    def from_mapping(cls, values: Mapping[str, Any]) -> "SSVMConfig":
        ...
        if unknown:
            raise ValueError(f"unknown SSVM configuration keys: {sorted(unknown)}")
        if "reconstruction_offsets" in data:
            data["reconstruction_offsets"] = tuple(data["reconstruction_offsets"])
        return cls(**data)

    @classmethod
    def load(cls, path: str | Path | None) -> "SSVMConfig":
        if path is None:
