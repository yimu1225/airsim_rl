...
def _build_vision_decoder(config: SSVMConfig) -> VisionMambaDecoder:
...
def _check_encoder_compatibility(
    vision_config: SSVMConfig, memory_config: SSVMConfig
) -> None:
    fields = (
        "image_height",
        "image_width",
...
def _save_grid(
    ...
    *,
    ...
    figure.suptitle(
        f"SSVM-SAC reconstruction — episode {episode_index}, t={time_index}"
    )
...
def visualize(args: argparse.Namespace) -> Path:
    ...
    vision_config = SSVMConfig.from_mapping(vision_checkpoint["config"])
    ...
    output = _save_grid(
        originals,
        reconstructions,
        args.output or _PROJECT_ROOT / "runs/SSVM_SAC/memory/reconstruction_test.png",
