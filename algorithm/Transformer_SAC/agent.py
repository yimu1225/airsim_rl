...
from .networks import Actor, Critic, TransformerEncoder


class TransformerSACAgent:
    """Transformer-SAC Agent: CNN spatial features + Transformer temporal modeling."""
    ...
    def __init__(self, base_dim: int, depth_shape, action_space, args, device=None, seed=None):
        ...
        if isinstance(self.ent_coef, str) and self.ent_coef.startswith("auto"):
            ...
            self.alpha = float(get_algo_param(args, "alpha", 0.2) if self.ent_coef is None else self.ent_coef)
            self.auto_entropy_tuning = False

        self.actor_encoder = TransformerEncoder(self.args).to(self.device)
        self.critic_encoder = TransformerEncoder(self.args).to(self.device)
        self.critic_encoder_target = TransformerEncoder(self.args).to(self.device)
    ...
    def train(self, progress_ratio=0.0):
        ...
        with torch.no_grad():
            next_actor_states = self._concat_state(next_base_states, next_depths, self.actor_encoder)
            next_target_states = self._concat_state(next_base_states, next_depths, self.critic_encoder_target)
        ...
        if self.total_it % self.policy_freq == 0:
            states_actor = self._concat_state(base_states, depths, self.actor_encoder)
            sampled_actions, log_probs, _, _ = self.actor(states_actor, with_log_prob=True)

            with torch.no_grad():
                critic_states_for_pi = self._concat_state(base_states, depths, self.critic_encoder)
