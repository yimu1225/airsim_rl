...
class GAMMambaTD3Agent:
    ...
    def train(self, progress_ratio=0.0):
        ...
        base_critic = self.critic_base_adapter(base_states)
        states_critic = torch.cat([base_critic, encoded_depths_critic], dim=1)

        with torch.no_grad():
            next_encoded_depths_critic = self._encode(next_depths, self.critic_encoder_target)
            next_base_critic = self.critic_base_adapter_target(next_base_states)
            next_states_critic = torch.cat([next_base_critic, next_encoded_depths_critic], dim=1)

            next_encoded_depths_actor = self._encode(next_depths, self.actor_encoder_target)
            next_base_actor = self.actor_base_adapter_target(next_base_states)
            next_states_actor = torch.cat([next_base_actor, next_encoded_depths_actor], dim=1)
        ...
        if self.total_it % self.policy_freq == 0:
            ...
            base_actor = self.actor_base_adapter(base_states)
            states_actor = torch.cat([base_actor, encoded_depths_actor], dim=1)

            with torch.no_grad():
                encoded_depths_critic_fixed = self.critic_encoder(depths)
                base_critic_fixed = self.critic_base_adapter(base_states)
                states_critic_fixed = torch.cat([base_critic_fixed, encoded_depths_critic_fixed], dim=1)
...
def make_agent(env, initial_obs, args, device=None) -> GAMMambaTD3Agent:
    base_state = initial_obs["base"]
    depth = initial_obs["depth"]
    agent = GAMMambaTD3Agent(
