...
from .networks import Actor, Critic, DepthEncoder, MotionEncoder, GRUEncoder, MetaNet, D_LGMD, BaseStateExpander
from .buffer import SequenceReplayBuffer


class D_LGMDGRUAETD3Agent:
    """
    AETD3 with GRU and D-LGMD for POMDP.
    ...
    def __init__(self, base_dim, depth_shape, gray_shape, action_space, args, device=None):
        ...
        print(f"AETD3-GRU-D-LGMD Agent using device: {self.device}")
        ...
        # Actor GRU
        self.actor_gru = GRUEncoder(expanded_base_dim=expanded_base_dim, depth_feature_dim=self.actor_depth_encoder.repr_dim, hidden_dim=self.gru_hidden_dim, num_layers=gru_layers).to(self.device)
        self.actor_gru_target = GRUEncoder(expanded_base_dim=expanded_base_dim, depth_feature_dim=self.actor_depth_encoder.repr_dim, hidden_dim=self.gru_hidden_dim, num_layers=gru_layers).to(self.device)
        self.actor_gru_target.load_state_dict(self.actor_gru.state_dict())
        
        # Critic GRU
        self.critic_gru = GRUEncoder(expanded_base_dim=expanded_base_dim, depth_feature_dim=self.critic_depth_encoder.repr_dim, hidden_dim=self.gru_hidden_dim, num_layers=gru_layers).to(self.device)
        self.critic_gru_target = GRUEncoder(expanded_base_dim=expanded_base_dim, depth_feature_dim=self.critic_depth_encoder.repr_dim, hidden_dim=self.gru_hidden_dim, num_layers=gru_layers).to(self.device)
        ...
        self.state_dim = self.gru_hidden_dim + self.actor_motion_encoder.repr_dim
        ...
        self.actor_optimizer = Adam(
            list(self.actor.parameters()) + list(self.actor_depth_encoder.parameters()) + list(self.actor_motion_encoder.parameters()) + list(self.actor_base_expander.parameters()) + list(self.actor_gru.parameters()), 
            lr=args.actor_lr
        )
        self.critic_optimizer = Adam(
            list(self.critic.parameters()) + list(self.critic_depth_encoder.parameters()) + list(self.critic_motion_encoder.parameters()) + list(self.critic_base_expander.parameters()) + list(self.critic_gru.parameters()), 
            lr=args.critic_lr
        )
    ...
    def select_action(self, base_seq, depth_seq, gray_seq, noise=True):
        ...
        with torch.no_grad():
            state = self._process_sequence(base, depth, gray, self.actor_depth_encoder, self.actor_motion_encoder, self.actor_gru, self.actor_base_expander)
    ...
    def train(self, progress_ratio=0.0):
        ...
        # Gray is uint8 from buffer, move to GPU then convert to float
        gray = torch.as_tensor(gray, device=self.device).float() / 255.0
        ...
        # Gray is uint8 from buffer
        next_gray = torch.as_tensor(next_gray, device=self.device).float() / 255.0
        ...
        state = self._process_sequence(base, depth, gray, self.critic_depth_encoder, self.critic_motion_encoder, self.critic_gru, self.critic_base_expander, detach_encoder=False)

        with torch.no_grad():
            # Critic Target Features
            next_state = self._process_sequence(next_base, next_depth, next_gray, self.critic_depth_encoder_target, self.critic_motion_encoder_target, self.critic_gru_target, self.critic_base_expander_target, detach_encoder=True)
            
            # Actor Target Features
            next_state_actor = self._process_sequence(next_base, next_depth, next_gray, self.actor_depth_encoder_target, self.actor_motion_encoder_target, self.actor_gru_target, self.actor_base_expander_target, detach_encoder=True)
        ...
        if self.total_it % self.policy_freq == 0:
            ...
            state_actor = self._process_sequence(base, depth, gray, self.actor_depth_encoder, self.actor_motion_encoder, self.actor_gru, self.actor_base_expander, detach_encoder=False)
            
            with torch.no_grad():
                state_critic = self._process_sequence(base, depth, gray, self.critic_depth_encoder, self.critic_motion_encoder, self.critic_gru, self.critic_base_expander, detach_encoder=False)
