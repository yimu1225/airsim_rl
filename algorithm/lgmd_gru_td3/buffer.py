...
class SequenceReplayBuffer:
    ...
    def add(self, base_state, depth, gray, action, reward, done):
        """
        Add a single step.
        """
        ...
        if self.base_buf is None:
            ...
            self.gray_shape = gray.shape   # (M, H, W)
            ...
            self.gray_buf = np.zeros((self.max_size, *self.gray_shape), dtype=np.float16) # Store gray as float16 for consistency
        ...
        # Store gray as float16 for consistency with depth
        self.gray_buf[self.ptr] = gray.astype(np.float16)
    ...
    def sample(self, batch_size: int):
        ...
        grays = self.gray_buf[seq_indices].astype(np.float32) # Convert back to float32
