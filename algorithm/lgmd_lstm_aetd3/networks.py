...
class DepthEncoder(nn.Module):
    """
    Depth Encoder for temporal processing with LSTM.
...
class LSTMEncoder(nn.Module):
    """
    Processes sequences frame-by-frame with LSTM.
    ...
    def __init__(self, expanded_base_dim, depth_feature_dim, hidden_dim, num_layers=1):
        super().__init__()
        # Only depth features go into LSTM
        lstm_input_dim = expanded_base_dim + depth_feature_dim
    ...
    def forward(self, base_expanded_seq, depth_visual_seq):
        ...
        combined = torch.cat([base_expanded_seq, depth_visual_seq], dim=2)  # (B, K, lstm_input_dim)
        
        # LSTM processing
        self.lstm.flatten_parameters()
        output, (hn, cn) = self.lstm(combined)
