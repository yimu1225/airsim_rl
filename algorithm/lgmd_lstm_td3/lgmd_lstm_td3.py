...
class D_LGMDLSTMTD3Agent:
    """
    TD3 with LSTM and D-LGMD for POMDP.
    ...
    def __init__(self, base_dim, depth_shape, gray_shape, action_space, args, device=None):
        ...
        print(f"TD3-LSTM-D-LGMD Agent using device: {self.device}")
        ...
        # LSTM Encoder - only processes depth features
        self.lstm_hidden_dim = args.lstm_hidden_dim
        lstm_layers = getattr(args, 'lstm_num_layers', 1)
        
        # Actor LSTM
        self.actor_lstm = LSTMEncoder(expanded_base_dim=expanded_base_dim, depth_feature_dim=self.depth_encoder.repr_dim, hidden_dim=self.lstm_hidden_dim, num_layers=lstm_layers).to(self.device)
        self.actor_lstm_target = LSTMEncoder(expanded_base_dim=expanded_base_dim, depth_feature_dim=self.depth_encoder.repr_dim, hidden_dim=self.lstm_hidden_dim, num_layers=lstm_layers).to(self.device)
        self.actor_lstm_target.load_state_dict(self.actor_lstm.state_dict())
        
        # Critic LSTM
        self.critic_lstm = LSTMEncoder(expanded_base_dim=expanded_base_dim, depth_feature_dim=self.depth_encoder.repr_dim, hidden_dim=self.lstm_hidden_dim, num_layers=lstm_layers).to(self.device)
        self.critic_lstm_target = LSTMEncoder(expanded_base_dim=expanded_base_dim, depth_feature_dim=self.depth_encoder.repr_dim, hidden_dim=self.lstm_hidden_dim, num_layers=lstm_layers).to(self.device)
        ...
        self.state_dim = self.lstm_hidden_dim + self.motion_encoder.repr_dim
        ...
        self.actor_optimizer = Adam(
            list(self.actor.parameters()) + list(self.depth_encoder.parameters()) + list(self.motion_encoder.parameters()) + list(self.actor_base_expander.parameters()) + list(self.actor_lstm.parameters()), 
            lr=args.actor_lr
        )
        self.critic_optimizer = Adam(
            list(self.critic.parameters()) + list(self.depth_encoder.parameters()) + list(self.motion_encoder.parameters()) + list(self.critic_base_expander.parameters()) + list(self.critic_lstm.parameters()), 
            lr=args.critic_lr
        )
    ...
    def select_action(self, base_seq, depth_seq, gray_seq, noise=True):
        ...
        with torch.no_grad():
            state = self._process_sequence(base, depth, gray, self.depth_encoder, self.motion_encoder, self.actor_lstm, self.actor_base_expander)
    ...
    def train(self, progress_ratio=0.0):
        ...
        state = self._process_sequence(base, depth, gray, self.depth_encoder, self.motion_encoder, self.critic_lstm, self.critic_base_expander, detach_encoder=False)

        with torch.no_grad():
            # Critic Target Features
            next_state = self._process_sequence(next_base, next_depth, next_gray, self.depth_encoder, self.motion_encoder, self.critic_lstm_target, self.critic_base_expander_target, detach_encoder=True)
            
            # Actor Target Features
            next_state_actor = self._process_sequence(next_base, next_depth, next_gray, self.depth_encoder, self.motion_encoder, self.actor_lstm_target, self.actor_base_expander_target, detach_encoder=True)
        ...
        if self.total_it % self.policy_freq == 0:
            ...
            state_actor = self._process_sequence(base, depth, gray, self.depth_encoder, self.motion_encoder, self.actor_lstm, self.actor_base_expander, detach_encoder=False)
            
            with torch.no_grad():
                state_critic = self._process_sequence(base, depth, gray, self.depth_encoder, self.motion_encoder, self.critic_lstm, self.critic_base_expander, detach_encoder=False)
