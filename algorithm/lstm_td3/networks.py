...
from ..cnn_modules import CNN, BaseStateExpander, BaseStateExpander
...
class LSTMEncoder(nn.Module):
    """
    Processes a sequence of visual features using LSTM frame-by-frame.
    Base state is not processed by LSTM, only concatenated at the end.
    """
    def __init__(self, visual_feature_dim, hidden_dim, num_layers=1):
        ...
        # LSTM input: only visual_feature
        self.lstm = nn.LSTM(visual_feature_dim, hidden_dim, num_layers, batch_first=True)
    ...
    def forward(self, visual_seq):
        ...
        for t in range(K):
            ...
            # LSTM forward for this timestep
            _, (h, c) = self.lstm(visual_t, (h, c))
