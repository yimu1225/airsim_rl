...
class VMambaRLTiny(nn.Module):
    ...
    参数量大幅减少，适合在线训练
    修改版：移除GlobalAvgPool，输出(B, L, C)保留空间信息用于Cross-Attention
    仅使用2个stage以降低计算复杂度
    """
    def __init__(self, 
...
class VMambaVisualEncoder(nn.Module):
    ...
    修改版：支持4帧通道堆叠输入，输出(B, L, C)用于Cross-Attention
    """
...
class FusionMLP(nn.Module):
    """
    Fuse visual and state features at each timestep.
    Input: visual_seq (B, K, V), state_seq (B, K, S)
    Output: fused_seq (B, K, H)
    """
    def __init__(self, visual_dim, state_dim, hidden_dim):
        super().__init__()
        self.fuse = nn.Sequential(
            nn.Linear(visual_dim + state_dim, hidden_dim),
            nn.LayerNorm(hidden_dim),
            nn.ReLU(inplace=True)
        )

    def forward(self, visual_seq, state_seq):
        x = torch.cat([visual_seq, state_seq], dim=-1)
        return self.fuse(x)
...
class CrossAttention(nn.Module):
    """
    交叉注意力机制：状态查询视觉特征
    Query: 来自无人机状态特征 state_feat (B, 1, dim)
    Key, Value: 来自VMamba输出的视觉特征 visual_feat (B, L, dim)
    Output: 融合后的特征向量 (B, dim)
    """
    def __init__(self, visual_dim, state_dim, hidden_dim, num_heads=4):
        super().__init__()
        ...
        # Query投影：将状态特征映射到hidden_dim
        self.query_proj = nn.Linear(state_dim, hidden_dim)
        # Key, Value投影：将视觉特征映射到hidden_dim
    ...
    def forward(self, visual_feat, state_feat):
        # visual_feat: (B, L, V) 来自VMamba的空间特征序列
        # state_feat: (B, S) 当前无人机状态向量
        
        B, L, V = visual_feat.shape
        S = state_feat.shape[-1]
        
        # 将状态特征转换为Query格式 (B, 1, hidden_dim)
        state_feat_expanded = state_feat.unsqueeze(1)  # (B, 1, S)
        q = self.query_proj(state_feat_expanded)  # (B, 1, hidden_dim)
        
        # 将视觉特征转换为Key, Value格式 (B, L, hidden_dim)
        k = self.key_proj(visual_feat)  # (B, L, hidden_dim)
        v = self.value_proj(visual_feat)  # (B, L, hidden_dim)
        
        # 交叉注意力：用状态查询视觉特征
        context, attention_weights = self.attention(q, k, v)  # (B, 1, hidden_dim)
        context = self.output_proj(context)  # (B, 1, hidden_dim)
...
class SimplifiedEncoder(nn.Module):
    """
    修正后的编码器：使用Cross-Attention融合视觉特征和无人机状态
    ...
    def forward(self, visual_feat, current_state):
        ...
        # 使用Cross-Attention融合视觉特征和状态特征
        fused_context = self.cross_attention(visual_feat, state_feat)  # (B, hidden_dim)
        
        # 输出投影
        fused_context = self.output_proj(fused_context)  # (B, hidden_dim)
