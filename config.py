...
def get_config(argv=None):
    ...
    parser.add_argument("--algorithm_name", type=str, default='CL-VSSM-SAC, CL-no-SB-PER, CL-no-VSSM, CL-SAC',
                        help="要训练的算法。支持: TD3, DDPG, VSSM-TD3, AETD3, SAC, SAC_FAE, LSTM-SAC, VSSM-SAC, no-SB-PER, no-VSSM, SVSSM-SAC, MM-VSSM-SAC, SAFE-VSSM-SAC, SB-PER-SVSSM-SAC, Transformer-SAC, SSVM-SAC（使用 algorithm/SSVM_SAC/train.py 分阶段训练）, SB-PER-MambaCSJA-SAC, PPO, VSSM-PPO, SDDPG。多个算法用逗号分隔，按填写顺序运行；CL- 前缀启用课程学习。")
