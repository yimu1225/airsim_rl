...
class AirLearningClient(object):
    ...
    def __init__(self, z, ip=None, port=None, config=None):
        ...
        # Depth image noise settings on the normalized 0-255 depth image.
        self.enable_depth_noise = bool(
            getattr(config, "enable_observation_noise", getattr(settings, "enable_depth_noise", True))
        )
        self.depth_gaussian_sigma = float(
            getattr(config, "depth_noise_gaussian_std", getattr(settings, "depth_gaussian_sigma", 3.0))
        )
        self.depth_gaussian_clip = float(
            getattr(config, "depth_noise_gaussian_clip", getattr(settings, "depth_gaussian_clip", 30.0))
        )
        salt_pepper_prob = getattr(config, "depth_noise_salt_pepper_prob", None)
        ...
        self.depth_motion_blur_kernel_size = max(
            1,
            int(getattr(config, "depth_noise_motion_blur_kernel_size", getattr(settings, "depth_motion_blur_kernel_size", 1))),
        )
    ...
    def _add_depth_noise(self, img):
        """
        Add salt-and-pepper, clipped Gaussian, and horizontal motion blur
        on the 0-255 depth image.
