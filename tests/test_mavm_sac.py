...
def _load_mavm_module(name):
    path = Path(__file__).parents[1] / "algorithm" / "MAVM_SAC" / f"{name}.py"
    spec = importlib.util.spec_from_file_location(f"_test_mavm_{name}", path)
...
def _load_mavm_package_module(name):
    ...
    return importlib.import_module(f"{package_name}.MAVM_SAC.{name}")


def test_episode_archive_round_trip_and_sequence_windows_do_not_cross_episodes(tmp_path):
    module = _load_mavm_module("dataset")
...
def test_reconstruction_targets_mask_missing_past_and_future_frames():
    build_reconstruction_targets = _load_mavm_module("dataset").build_reconstruction_targets
...
def test_default_reconstruction_targets_are_t_minus_5_t_minus_10_and_t_plus_5():
    build_reconstruction_targets = _load_mavm_module(
        "dataset"
...
def test_config_projects_one_reconstruction_code_per_offset():
    config_module = _load_mavm_package_module("config")
    config_path = Path(__file__).parents[1] / "algorithm" / "MAVM_SAC" / "params.yaml"

    config = config_module.MAVMConfig.load(config_path)
...
def test_final_stage_curve_logger_writes_vssm_sac_compatible_csv(tmp_path):
    metrics_module = _load_mavm_package_module("metrics")
    logger = metrics_module.FinalStageCurveLogger(
        algorithm_name="MAVM-SAC",
        seed=25,
    ...
    assert logger.run_dir == tmp_path / "MAVM-SAC" / "seed25"
    assert rows == [
...
def test_final_stage_tensorboard_uses_the_formal_results_run_directory(tmp_path):
    train_module = _load_mavm_package_module("train")
    metrics_module = _load_mavm_package_module("metrics")
    logger = metrics_module.FinalStageCurveLogger(
        algorithm_name="CL-MAVM-SAC",
        seed=25,
        results_root=tmp_path,
    )
...
def test_shared_curve_plotter_recognizes_mavm_sac_name():
    from algo_name_utils import expand_algorithm_spec, to_output_algorithm_name

    assert expand_algorithm_spec("MAVM-SAC") == ["MAVM_SAC"]
    assert to_output_algorithm_name("CL-MAVM_SAC") == "CL-MAVM-SAC"
...
def test_memory_stage_uses_new_offsets_with_an_existing_vision_checkpoint():
    train_module = _load_mavm_package_module("train")
    config_module = _load_mavm_package_module("config")
    vision_checkpoint_config = config_module.MAVMConfig(
        reconstruction_offsets=(-2, -1, 1)
    ).to_dict()
    requested_config = config_module.MAVMConfig(
...
def test_epoch_metrics_weight_unequal_batches_by_pixel_count():
    module = _load_mavm_package_module("train")
...
def test_validation_restores_training_mode_and_excludes_missing_targets():
    module = _load_mavm_package_module("train")
    config = _load_mavm_package_module("config").MAVMConfig(reconstruction_offsets=(-1, 1))
...
def test_memory_cosine_schedule_uses_yaml_ratio_and_epoch_horizon():
    module = _load_mavm_package_module("train")
    config_module = _load_mavm_package_module("config")
    old = config_module.MAVMConfig(memory_lrf=0.5).to_dict()
    requested = config_module.MAVMConfig(memory_lr=0.02, memory_lrf=0.01)
...
def test_memory_stage_overrides_old_loss_with_requested_mse():
    module = _load_mavm_package_module("train")
    config_module = _load_mavm_package_module("config")
    old = config_module.MAVMConfig(reconstruction_loss="charbonnier").to_dict()
    result = module._memory_stage_config(old, config_module.MAVMConfig())
...
def test_memory_stage_uses_requested_learning_rate_and_decay():
    module = _load_mavm_package_module("train")
    config_module = _load_mavm_package_module("config")
    old = config_module.MAVMConfig(
        latent_dim=64, memory_lr=0.004, offline_lr_decay=0.95,
    ).to_dict()
    requested = config_module.MAVMConfig(
...
def test_memory_visualizer_selects_a_time_where_every_offset_is_valid():
    module = _load_mavm_package_module("visualization.visualize_memory_reconstruction")
...
def test_memory_visualizer_uses_project_training_paths_without_arguments():
    ...
    assert Path(args.dataset) == project_root / "datasets" / "MAVM_SAC"
    assert Path(args.vision_checkpoint) == (
        project_root / "runs" / "MAVM_SAC" / "vision" / "vision_latest.pt"
    )
    assert Path(args.memory_checkpoint) == (
        project_root / "runs" / "MAVM_SAC" / "memory" / "memory_latest.pt"
    )
...
def test_temporal_memory_online_step_matches_full_causal_forward():
    TemporalMambaMemory = _load_mavm_module("networks").TemporalMambaMemory
...
def test_vision_mamba_decoder_unpatchifies_one_reconstruction_latent():
    VisionMambaDecoder = _load_mavm_module("networks").VisionMambaDecoder
...
def test_linear_patch_expand_places_children_in_spatial_order():
    module = _load_mavm_module("networks")
...
def test_direct_split_checkpoint_version_compatibility(tmp_path):
    module = _load_mavm_package_module("checkpoints")
...
def test_feature_replay_buffer_returns_fixed_representation_transitions():
    FeatureReplayBuffer = _load_mavm_package_module("buffer").FeatureReplayBuffer
...
def test_dual_prioritized_feature_replay_routes_complete_episodes_by_success():
    buffer_module = _load_mavm_package_module("buffer")
...
def test_sac_update_uses_frozen_memory_features_without_encoder_gradients():
    agent_module = _load_mavm_package_module("agent")
    config_module = _load_mavm_package_module("config")
    config = config_module.MAVMConfig(
        memory_dim=3,
        hidden_dim=16,
        replay_capacity=16,
        batch_size=4,
    )
    agent = agent_module.MAVMSACAgent(
        _FakePerception(3),
        base_dim=2,
